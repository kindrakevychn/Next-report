...
const AUTH = 'Bearer e2a905ea7d9d04c14d3127430d300550';

/**
 * See Valurank documentation for `GET /content`.
 *
 * @returns
 * Data if found, `null` if not found.
 *
 * @throws
 * Throws error in case of failed request (except HTTP 404).
 */
export async function getContent(id) {
    const response = await fetch(`https://api.valurank.com/v1/content/${id}`, {
        method: 'GET',
        headers: {
            'Authorization': AUTH,
...
export function getReportData() {
    ...
    // to get report data.
    //
    // `window.vrReportData` have exact structure as `data`.
    if (window.vrReportData != null) {
        return window.vrReportData;
    }
    ...
    // We always expect that `window.vrReportData` is already embedded.
